...
import java.net.{InetSocketAddress, URI, URLClassLoader}
 ...
 */
case class Forker(javaEnv: JavaEnv, classpath: Array[AbsolutePath]) {
...
object Forker {
  ...
  def forJvm(javaEnv: JavaEnv, classpath: Array[AbsolutePath]): Forker = {
    ???
  }

  final class JvmForker(javaEnv: JavaEnv, classpath: Array[AbsolutePath])
      extends Forker(javaEnv, classpath) {
    override def runMain(
      ...
      val logger = new JvmProcessLogger(logger0)
      super.runMain(cwd, mainClass, args0, skipJargs, logger, opts, extraClasspath)
    }
  }
